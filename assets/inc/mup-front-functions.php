...
function mup_footer_credit() {
	...
	if( !empty( $credit ) ) {
		...
		echo '<a href="https://wordpress.org/">Proudly powered by WordPress</a> &amp; <a href="http://musicpress.io">MusicPress</a>';
	}
}
add_action( 'wp_footer', 'mup_footer_credit', 100 );
...
/**
 * Multiple shortcode Gigs - [mup-gigs]
 * @since  1.0
 */
...
/**
 * Multiple shortcode Tracks - [mup-tracks]
 * @since  1.0
 */
...
/**
 * Singular shortcode single Gig - [mup-gig id=""]
 * @since  1.0
 */
...
/**
 * Singular shortcode Track - [mup-track id=""]
 * @since  1.0
 */
...
/**
 * Shortcode Social - [mup-social type=""]
 * @since  1.0
 */
...
/**
 * Booking Shortcode - [mup-booking-form email=""] 
 * @since  1.2
 */
function mup_booking_form_shortcode( $atts ) {

	$email = shortcode_atts(array(
      'email' => 'Returning nothing as nothing valid was passed',
   	), $atts);
	...
	if( !is_email( $email['email'] ) ) {
   	...
   	$mup_booking_form = '';
   	$mup_booking_form .= '
	<div class="mup-booking-form">
			<div class="mup-success">
				<span class="message">' . __( 'Booking request sent.', 'mup' ) . '</span>
			</div>
			<div class="mup-error">
				<span class="error-title">' . __( 'Booking Errors', 'mup' ) .'</span>
				<br><span class="message"></span>
			</div>
			<div class="mup-booking-input">
		   		<input type="text" id="mup-booking-name" name="mup-booking-name" placeholder="' . __( 'Name', 'mup' ) . '">
		   	</div>
		   	<div class="mup-booking-input">
		   		<input type="email" id="mup-booking-email" name="mup-booking-email" placeholder="' . __( 'Email', 'mup' ) . '">
		   	</div>
		   	<div class="mup-booking-input half-width">
		   		<input type="text" id="mup-booking-date" name="mup-booking-date" placeholder="' . __( 'Event Date', 'mup' ) . '">
		   	</div>
		   	<div class="mup-booking-input half-width">
		   		<input type="text" id="mup-booking-website" name="mup-booking-website" placeholder="' . __( 'URL (optional)', 'mup' ) . '">
		   	</div>
		   	<div class="mup-booking-input">
		   		<textarea name="mup-booking-message" id="mup-booking-message" placeholder="' . __( 'Details', 'mup' ) . '"></textarea>
		   	</div>
		   	<div class="mup-booking-input">
		   		<input type="hidden" name="mup_booking_form_nonce" id="mup_booking_form_nonce" value="' . wp_create_nonce( 'mup_booking_form_nonce' ) .'" />    
		   		<input type="submit" class="mup-booking-form-submit" id="mup-booking-form-submit" name="mup-booking-form-submit" value="' . __( 'Send', 'mup' ) . '">
		   	</div>
	</div>
	';
   	return $mup_booking_form;
}
add_shortcode( 'mup-booking-form', 'mup_booking_form_shortcode' );

/**
 * Sends the booking requerst via Ajax
 * @since 1.2
 */
function mup_send_booking_form() {

	if( check_ajax_referer( 'mup_booking_form_nonce', 'nonce', false ) ) { 
		...
		// Check if empty
		if( empty( $name ) ) {
			$errors[] = __( 'Please enter Name', 'mup' );
		}

		if( empty( $email ) ) {
			$errors[] = __( 'Please enter Email', 'mup' );
		}
		elseif( filter_var( $email, FILTER_VALIDATE_EMAIL ) === false ) {
		
			$errors[] = __( 'Invalid Email', 'mup' );
		}

		if( empty( $date ) ) {
			$errors[] = __( 'Please enter Date', 'mup' );
		}

		if( empty( $message ) ) {
			$errors[] = __( 'Please enter Details', 'mup' );
		}
		...
		if( !empty( $errors ) ) {
			...
			$subject = $name . ' - ' . __( 'Booking Request', 'mup' );
            ...
            $content  = $message . "\r\n\r\n";
            $content .=	__( 'From', 'mup' ) . ": $name\r\n\r\n";
            $content .=	__( 'Email', 'mup' ) . ": $email\r\n\r\n";

            if( !empty( $website ) ) {
            	$content .=	__( 'URL', 'mup' ) . ": $website\r\n";
            }	
             
            $mup_send_booking_request = wp_mail( $to, $subject, $content, $headers );

            if( $mup_send_booking_request ) {

            	$errors[] = __( 'success', 'mup' );
    			$out = array_values( $errors );
				echo json_encode( $errors );
    			die();
    	...
    	$errors[] = __( 'Form submission error', 'mup' );
    	$out = array_values( $errors );
		echo json_encode( $errors );
    	die();
    }
...
add_action( 'wp_ajax_nopriv_mup_send_booking_form', 'mup_send_booking_form' ); 
add_action( 'wp_ajax_mup_send_booking_form', 'mup_send_booking_form' );
